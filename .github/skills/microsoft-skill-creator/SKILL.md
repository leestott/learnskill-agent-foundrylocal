...
compatibility: Requires Microsoft Learn MCP Server (https://learn.microsoft.com/api/mcp). Related skills - microsoft-docs (concept lookups), microsoft-code-reference (API verification), repo-onboarding-pack (repository documentation).
---
...
4. Cross-check: Use `microsoft_docs_search` to confirm claims are accurate
...
## Validation Against Microsoft Learn

All generated skills MUST be validated against official documentation:

### Validation Workflow

1. **Fact-check claims** — For each key concept, run:
   ```
   microsoft_docs_search(query="{technology} {concept}")
   ```
2. **Verify code patterns** — For each code sample:
   ```
   microsoft_code_sample_search(query="{technology} {pattern}", language="{lang}")
   ```
3. **Confirm versions** — Check current versions:
   ```
   microsoft_docs_search(query="{technology} what's new latest version")
   ```
4. **Cross-reference best practices**:
   ```
   microsoft_docs_search(query="{technology} best practices")
   ```
...
- [ ] Every technology name matches official docs naming
- [ ] SDK versions are current (not deprecated)
- [ ] API patterns match latest best practices
- [ ] Code samples compile/run without errors
- [ ] "Learn More" queries return relevant results
- [ ] No hallucinated methods or classes
- [ ] Configuration options exist in official docs
